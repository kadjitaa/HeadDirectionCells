...
def computeFiringRates(spikes, epochs, name, tc):
    ...
    for n, ep, tc, ang in zip(name, epochs,tc):
        for k in spikes:
...
def computeCircularStats(epochs,spikes,position, names):
    circ_mean=pd.DataFrame(index=spikes.keys(), columns=name)
    circ_var=pd.DataFrame(index=spikes.keys(), columns=name)
    for n,ep in zip (name,epochs):
        for k in spikes:   
